...
def process_bulk_images(folder_path, save_option):
    """
    Process all images in the selected folder, extract text, and save to files based on the selected save option.
    ...
    output_file_path = os.path.join(output_folder, "all_extracted_texts.txt")
    ...
    for i, file_name in enumerate(files):
        ...
        if save_option == "single_file":
            ...
            text_file_path = os.path.join(output_folder, f"{os.path.splitext(file_name)[0]}.txt")
            with open(text_file_path, "w", encoding="utf-8") as text_file:
                text_file.write(extracted_text)
        elif save_option == "extracted_name":
            if extracted_text.strip():
                valid_filename = extracted_text.split('\n')[0][:50].replace(" ", "_").replace("/", "_") + ".txt"
                text_file_path = os.path.join(output_folder, valid_filename)
                with open(text_file_path, "w", encoding="utf-8") as text_file:
                    text_file.write(extracted_text)
...
def start_bulk_processing():
    ...
    if folder_path:
        ...
        threading.Thread(target=process_bulk_images, args=(folder_path, save_option), daemon=True).start()
...
single_file_radio = tk.Radiobutton(options_frame, text="Save All in Single File", variable=save_option_var, value="single_file")
...
extracted_name_radio = tk.Radiobutton(options_frame, text="Save as Extracted Names", variable=save_option_var, value="extracted_name")
